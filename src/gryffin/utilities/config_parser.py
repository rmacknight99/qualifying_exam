...
class ConfigParser(Logger):
    ...
    def __init__(self, config_file=None, config_dict=None, discrete_increment=None):

        Logger.__init__(self, 'ConfigParser', verbosity=3)
        self.category_parser = CategoryParser()
        self.config_file     = config_file
        self.config_dict     = config_dict
        self.increment       = discrete_increment
    ...
    def _parse_parameters(self, provided_settings):
        ...
        for setting in provided_settings:
            ...
            if setting['type'] == 'continuous':
                ...
                if setting['high'] <= setting['low']:
                    GryffinValueError('upper limit (%f) needs to be larger than the lower limit (%f) for parameter "%s"' % (setting['high'], setting['low'], setting['name']))
                setting['options']     = [round(j,1) for j in np.arange(setting["low"], setting["high"] + self.increment[d], self.increment[d]).tolist()]  # +1 to use closed interval
                setting['descriptors'] = [round(j,1) for j in np.arange(0, setting['high'] - setting['low'] + self.increment[d], self.increment[d]).tolist()]  # +1 to use closed interval
